...
#pragma warning restore SA1401 // Fields should be private
    ...
    public override TEnum Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        if (token == JsonTokenType.String)
        {
            ...
            return _DeserializationFailureFallbackValue ?? throw new JsonException($"The JSON value '{enumString}' could not be converted to {_EnumType}.");
        }
    ...
    public override TEnum ReadAsPropertyName(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        if (token != JsonTokenType.PropertyName)
            throw new JsonException($"he JSON value could not be converted to {_EnumType}.");
        ...
        if (ulong.TryParse(enumString, out ulong numericValue))
        {
            return !_AllowIntegerValues
                ? _DeserializationFailureFallbackValue ?? throw new JsonException($"he JSON value could not be converted to {_EnumType}.")
    ...
    private bool TryGetStringForFlagsEnumValue(
        ...
        [NotNullWhen(true)] out string? flagsValueString)
    {
        ulong calculatedValue = 0;
